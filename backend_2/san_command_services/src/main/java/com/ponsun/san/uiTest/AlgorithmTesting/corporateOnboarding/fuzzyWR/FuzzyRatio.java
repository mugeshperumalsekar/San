...
public class FuzzyRatio {
    public static double main(String name1,String name2,List<String> token1,List<String> token2,double weight) {
        ...
        IntStream.range(0, token1.size()).parallel().forEach(i -> {
            ...
            for (int j = 0; j < token2.size(); j++) {
                ...
                sim[i][j] = (double) Math.round(me.xdrop.fuzzywuzzy.FuzzySearch.ratio(t1, t2));
//	                // // System.out.println("token1: " +t1+"  token2: " +t2+" "+sim[i][j]);
            }
        });
...
//	        // System.out.println("match1 modified fuzzy:"+matchingper);
        double matchingper2 = (double) Math.round(me.xdrop.fuzzywuzzy.FuzzySearch.ratio(name1,name2)) ;
        if(matchingper<matchingper2) {
            matchingper= matchingper2;
        }

//	        // System.out.println("match2 fuzzy:"+matchingper2);
