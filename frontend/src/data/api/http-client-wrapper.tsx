...
class HttpClientWrapper {
  ...
  public xmlpost = async (path: string, payload: any, config = {}) => {
    console.log("HttpClientWrapper xmlpost() start path = '" + path + "', payload = ", payload);
    try {
      let response: any = await this.axiosClient.post(path, payload, config);
      console.log("HttpClientWrapper xmlpost() response data ", response.data);
      console.log("HttpClientWrapper xmlpost() end");
      return response.data;
    } catch (err: any) {
      console.error("HttpClientWrapper xmlpost() error: ", err);
      toast.error(err.response?.data?.message || "An error occurred", { containerId: 'TR' });
      throw err;
    }
  };
